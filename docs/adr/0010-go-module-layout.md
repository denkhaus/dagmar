...
domain core, application services, adapters (Dagger, Kubernetes, observability), config,
...
passes the Project source at construction, and every method reuses it.
...
ports.~~ — **Superseded by ADR-0018**: the Go port layer is removed; Project Hook Services
are Dagger functions, not Go interfaces. The pure decision logic (gate/merge/prompt rules) lives in `domain/` with **zero Dagger
...
The module's main object is `Dagmar` (auto-named from the module). `New(project *dagger.Directory) *Dagmar` is the constructor and the per-Project binding
seam (ADR-0018: ~~`seeds, mulch, canopy string`~~ removed; only `project` remains). All
...
- `app/` port-logic — unit-tested against `mockgen`-generated mocks of the remaining port
  (Tracer; `go.uber.org/mock`, per KB `guide.golang.testing`; in-package mock via
  `ports/generate.go`). The tool is a tracked module dependency (`tools.go`, `//go:build tools`)
  and `mock_tracer.go` is committed, so a fresh checkout resolves it from the module cache — no
  live network at `go generate` time. (ADR-0018: the os-eco port mocks are removed; only Tracer
  survives.)
...
- Config: `envconfig`-style struct tags for runtime/env (the env resolver is not yet wired —
  ...
  ADR-0003) for declarative per-Project conformance. ~~The two same-shape os-eco triples reconcile at runtime: the constructor-seam binding
  (`main.OsEcoBinding`, from the ProjectManifest) is authoritative for a Run's store paths;
  `config.OsEcoConfig` is the env-override view of the same triple.~~ — **Superseded by
  ADR-0018**: `OsEcoBinding` and `OsEcoConfig` are removed; Project Hook Services are Dagger
  functions, not bound at construction.
...
- **A custom struct as a constructor ARG is skipped.** ~~os-eco paths are passed as primitive
  strings and grouped into the `OsEcoBinding` field internally.~~ — **ADR-0018**: os-eco
  constructor args removed entirely; `New()` takes only `project`.
...
- **Landed:** 13 hand-written files + `mock_oseco.go` (generated, committed; ADR-0018:
  replaced by `mock_tracer.go`) + regenerated bindings; `go build`/`test`/`vet` green; `dagger develop` clean; the `Sandbox` vertical
...
- **ADR links:** ADR-0001 (Tier A direct, Tier B ports) confirmed; ADR-0003 (ProjectManifest =
  declarative config); ADR-0005 (~~os-eco ports~~ → superseded by ADR-0018); ADR-0008 (constructor binds per-Project for the
  `kube-pod://` client flow).
- **Deferred:** `Run`/`Gate`/`Workspace` methods; ~~os-eco adapter implementations (sd/ml/cn)~~ → removed
  (ADR-0018);
